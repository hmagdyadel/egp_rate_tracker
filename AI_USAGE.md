...
### Phase 8 — Multi-range chart & UX polish — 2026-07-23 (commit `6de8aec`)
...
- **Loading UI Iterations:** Iterated chart loading state three times: dummy chart (looked static/wrong) → hand-drawn line skeleton (visually mismatched) → settled on `SpinKitThreeBounce` `Loader` centered inside `AspectRatio(1.6)` container to prevent layout jump.
